...
function shell(){
  $('#syllabus').innerHTML=catalog.blocks.map(b=>`<details class="block-nav" ${route().startsWith(b.id+'.')?'open':''}><summary><span class="block-number">${b.id}</span><span>${esc(b.shortTitle)}</span><span class="chevron">⌄</span></summary><div>${catalog.topics.filter(t=>t.blockId===b.id).map(t=>`<a href="#tema/${t.id}" ${route()===t.id?'aria-current="page"':''}><span>${t.id}</span>${esc(t.title)}<i class="${t.videos.length?'has-video':'no-video'}" title="${t.videos.length?'Con vídeos':'Sin vídeo localizado'}"></i></a>`).join('')}</div></details>`).join('');
...
function renderResults(){
  ...
  let count=0;
  const result=topics.map(t=>{const links=selectVideos(t,catalog,filters,state);count+=links.length;if(!links.length)return '';return `${current==='biblioteca'?`<div class="result-topic"><a href="#tema/${t.id}">${t.id} · ${esc(t.title)} ↗</a><span>${links.length} vídeos</span></div>`:''}${links.map(l=>videoCard(l,t.videos.indexOf(l))).join('')}`;}).join('');
  $('#results').innerHTML=result||`<div class="empty"><span aria-hidden="true">▤</span><h3>${topics.length===1&&!topics[0].videos.length?'Todavía no hay un vídeo localizado para este tema':'No hay vídeos con estos filtros'}</h3><p>${topics.length===1&&!topics[0].videos.length?'El tema permanece en su lugar del programa. Consulta el alcance pendiente y utiliza las notas para organizar tu estudio.':'Prueba otra búsqueda o pulsa «Limpiar».'}</p></div>`;
...
function library(topic){
  ...
  const head=topic?`<div class="eyebrow">BLOQUE ${topic.blockId} · TEMA ${topic.id}</div><h1>${esc(topic.title)}</h1><details class="official"><summary>Ver enunciado oficial completo</summary><p>${esc(topic.officialTitle)}</p></details><div class="topic-coverage"><span class="badge ${topic.videos.length?'partial':'missing'}">${topic.videos.length?'Cobertura parcial':'Sin vídeo localizado'}</span><p>${esc(topic.coverageNote)}</p></div>`:`<div class="eyebrow">TU BIBLIOTECA</div><h1>Una explicación para<br>cada paso.</h1><p class="intro">Paco Barbié y otros autores, organizados según el ANEXO II.<br>Elige un tema o encuentra la explicación que necesitas.</p>`;
  $('#main').innerHTML=`<section class="library-heading">${head}</section>${toolbar()}<div class="results-header"><h2>${topic?'Ruta de vídeos del tema':'Explorar vídeos'}</h2><span id="result-count"></span></div><section id="results" aria-label="Vídeos encontrados"></section>${topic?`<section class="notes"><div class="section-heading"><h2>Mis notas · ${topic.id}</h2><span id="note-status" role="status">Guardadas en este dispositivo</span></div><label for="topic-note">Lo que quiero recordar de este tema</label><textarea id="topic-note" data-topic="${topic.id}" maxlength="50000" placeholder="Anota dudas, artículos clave o lo que quieres repasar…">${esc(state.notes[topic.id]||'')}</textarea><p>Las notas no se publican en GitHub. Puedes guardarlas en una copia desde los ajustes.</p></section><nav class="topic-pagination" aria-label="Cambiar de tema">${neighbor(topic,-1)}${neighbor(topic,1)}</nav>`:''}`;
  renderResults();
}
...
document.addEventListener('change',e=>{if(['channel','status'].includes(e.target.id)){filters[e.target.id]=e.target.value;renderResults();}});
